...
#define CHMODE 0 /* CHannel MODE 0=mono, 1=dual, 2=stereo, 3=joint */
#define ALLOCMETHOD 0 /* bit ALLOCation METHOD 0=loudness, 1=SNR */
...
*
* Purpose: Compute the sign correlation according to Eq. (5) of Goodman paper
*
* Inputs: *x - pointer to x input vector
* *y - pointer to y input vector
*
* Outputs: Sn - return value containing the sign-correlation of x and y
*
*****************************************************************************/
short SignCorrelation(short *x, short *y)
{
	short Sn = 0;
	int m = 0;
	for (m=0;m<M;m++)
...
short PatternMatchS(short *y)
{
	short n;
	short maxSn=-32765; /* large negative number */
	short Sn;
	short bestmatch = 0;
	...
	for (n=0;n<N;n++)
	{
		Sn = SignCorrelation(&y[LHIST-M] /* x */, &y[n]);
		if (Sn>maxSn)
		{
			bestmatch=n;
			maxSn = Sn;
...
short GetMiddle(short * y, short size)
{
	short Max = - 32765;
	short Min = 32765;
	int sum = 0;
...
short GetAverage(short * y, short size)
{
	int sum = 0;
	for (int i = 0; i < size; ++i)
	{
...
short RelativeCorrelation(short *x, short *y, short r)
{
	short Sn = 0;
	int m = 0;
	for (m=0;m<M;m++)
...
int PatternMatchSignMiddle(short *y)
{
	short n;
	short maxSn=-32765; /* large negative number */
	short Sn;
	...
	short Middle = GetMiddle(&y[LHIST-M], M);
	TRACE("Middle(%d)\n", Middle);
	for (n=0;n<N;n++)
	{
		Sn = RelativeCorrelation(&y[LHIST-M] /* x */, &y[n], Middle);
		if (Sn>maxSn)
		{
			bestmatch=n;
			maxSn = Sn;
...
int PatternMatchSignAverage(short *y)
{
	short n;
	short maxSn=-32765; /* large negative number */
	short Sn;
	short bestmatch = 0;
	short Average = GetAverage(&y[LHIST-M], M);
	TRACE("Average(%d)\n", Average);
	for (n=0;n<N;n++)
	{
		Sn = RelativeCorrelation(&y[LHIST-M] /* x */, &y[n], Average);
		if (Sn>maxSn)
		{
			bestmatch=n;
			maxSn = Sn;
...
#define  REDUCTION 0.6
    ...
    if (plc_state->nbf==1)
    {
		...
		int nCorssMatch = PatternMatch(plc_state->hist);
		int nSignMatch = PatternMatchS(plc_state->hist);
		int nMiddleMatch = PatternMatchSignMiddle(plc_state->hist);
		int nAverageMatch = PatternMatchSignAverage(plc_state->hist);
		TRACE("CorssMatch(%d)SignMatch(%d)MiddleMatch(%d)AverageMatch(%d)\n", 
			nCorssMatch, nSignMatch, nMiddleMatch, nAverageMatch);

		if (abs(nCorssMatch-nSignMatch) < 10)
		{
			TRACE("SignGood\n");
		} 
		else
		...
		if (abs(nCorssMatch-nMiddleMatch) < 10)
		{
			TRACE("MiddleGood\n");
		} 
		else
		...
		if (abs(nCorssMatch-nAverageMatch) < 10)
		{
			TRACE("AverageGood\n");
		} 
		else
    ...
    else
    {
		for (;i<FS+SBCRT+OLAL;i++)
		{
			plc_state->hist[LHIST+i] = (short)(plc_state->hist[plc_state->bestlag+i] * REDUCTION);
		}
    }
    for (i=0;i<FS;i++)
