...
CWaveDoc::CWaveDoc(CString &PathName)
{
	fileOpened = FALSE;
...
void CWaveDoc::Serialize(CArchive& ar)
{
	...
	else
	{
		...
		ar >> fileLen;	// fileLen Õ¼4¸ö×Ö½Ú
		if (fileLen != fileSize - 8) {
			MessageBox(NULL, L"Wrong file size!", L"Error", MB_OK);
			throw(new CFileException(CFileException::generic));
		}
		...
		ar >> id;
		if (id != ID_WAVE) {
			MessageBox(NULL, L"Not WAVE format!", L"Error", MB_OK); 
			throw(new CFileException(CFileException::generic));
		}
		...
		while (!format || !data || pos < fileSize) {
			...
			if (id == ID_fmt) { // read format chunk
				ar >> formatTag;
				if (formatTag != 1) {
					MessageBox(NULL, L"Not PCM format!", L"Error", MB_OK); 
					throw(new CFileException(CFileException::generic));
				}
				ar >> channels;
				ar >> sampleRate;
				...
				if (!format) {
					MessageBox(NULL, L"No format chunk before data chunk!", L"Error", MB_OK);
					throw(new CFileException(CFileException::generic));
				}
				// validate and calculate parameters
				if (bitsPerSample <= 0) {
					MessageBox(NULL, L"Wrong bits per sample!", L"Error", MB_OK); 
					throw(new CFileException(CFileException::generic));
				}
				bytes = (WORD)((bitsPerSample + 7) / 8);
				if (bytes > 2) {
					MessageBox(NULL, L"Too large bits per sample!", L"Error", MB_OK);
					throw(new CFileException(CFileException::generic));
				}
				n = chkLen / (channels * bytes);
				if (n <= 0) {
					MessageBox(NULL, L"Sample number <= 0!", L"Error", MB_OK);
					throw(new CFileException(CFileException::generic));
				}
		...
		//ar.Close();
		if (!format || !data) {
			MessageBox(NULL, L"Wrong file data!", L"Error", MB_OK); 
			throw(new CFileException(CFileException::generic));
		}
