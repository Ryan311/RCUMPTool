...
aes_context ctx;
BOOL g_bAesEnable = FALSE;

const unsigned char OTA_Secret_key[32] =
{ 
...
void OTA_AES_Init( int aes_mode )
{
	...
	memcpy(key, OTA_Secret_key, 16 + n * 8);
...
#define random(x) (rand()%x)	//产生[0, x)区间内的随机整数
	...
	// 产生16个随机数
	for( int i = 0;  i < 16; i++ )
		RandData[i] = random(255); // 产生16个[0, 255)之间的随机数

	UINT8 TestData1[16] = {0x01, 0x02, 0x01, 0x02, 0x01, 0x02, 0x01, 0x02,
		0x01, 0x02, 0x01, 0x02, 0x01, 0x02, 0x01, 0x02};
	for( int i = 0;  i < 16; i++ )
		RandData[i] = TestData1[i]; // 产生16个[0, 255)之间的随机数
}
...
PATCH_SRAM_OFF_BD_BSS_SECTION uint32_t KT0[256];
PATCH_SRAM_OFF_BD_BSS_SECTION uint32_t KT1[256];
PATCH_SRAM_OFF_BD_BSS_SECTION uint32_t KT2[256];
PATCH_SRAM_OFF_BD_BSS_SECTION uint32_t KT3[256];
#endif
